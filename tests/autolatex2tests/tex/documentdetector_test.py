...
class TestDocumentDetectorFromString(AbstractBaseTest):
	@override
	def setUp(self):
...
class TestDocumentDetectorFromFile(AbstractBaseTest):
	...
	def test_atBegining(self):
		self.__run(True, '\\documentclass{article}\\begin{document}\\end{document}')

	def test_withComment1(self):
		self.__run(False, '%Mycomment\\documentclass{article}\\begin{document}\\end{document}')

	def test_withComment2(self):
		self.__run(True, "%Mycomment\n\\documentclass{article}\\begin{document}\\end{document}")

	def test_withMath(self):
		self.__run(True, '\\begin{document}$A_1$\\end{document}\\documentclass{article}')
