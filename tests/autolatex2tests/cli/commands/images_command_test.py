...
class TestImagesAction(AbstractCommandTest):

	def __init__(self, x):
		super().__init__(x, 'images')
	...
	def test_no_target_images(self):
		self.do_test(config=self._config)
		self.assertTrue(os.path.isfile(self._target_img1))
		self.assertTrue(os.path.isfile(self._target_img2))
		self.assertTrue(os.path.isfile(self._target_img3))
