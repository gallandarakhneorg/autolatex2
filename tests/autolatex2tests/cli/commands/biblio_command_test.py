...
class TestBiblioAction(AbstractCommandTest):

	def __init__(self, x):
		super().__init__(x, 'biblio')
	...
	def _initialize_test_folder(self):
		...
		self._bbl_file = os.path.normpath(os.path.join(self._tmp_folder_name, 'rootfile.bbl'))

		shutil.copyfile(os.path.normpath(os.path.join(self._resource_directory, 'test21.bib')),
						os.path.normpath(os.path.join(self._tmp_folder_name, 'rootfile.bib')))
		shutil.copyfile(os.path.normpath(os.path.join(self._resource_directory, 'test21.tex')),
						self._tex_file)
	...
	def test_w_latex_compilation(self):
		...
		self.do_test(config=self._config)
		self.assertTrue(os.path.isfile(self._bbl_file))
