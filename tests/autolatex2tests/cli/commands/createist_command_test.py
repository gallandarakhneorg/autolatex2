...
class TestCreateIstAction(AbstractCommandTest):

	def __init__(self, x):
		super().__init__(x, 'createist')
	...
	def _initialize_test_folder(self):
		...
		self._ist_file = os.path.normpath(os.path.join(self._tmp_folder_name, 'default.ist'))
	...
	def test_no_existing_file(self):
		...
		self.assertTrue(os.path.isfile(self._ist_file), "IST file %s not found" % self._ist_file)


	def test_existing_file_no_force(self):
		with open(self._ist_file, "wt") as out_file:
			out_file.write("test\n")
	...
	def test_existing_file_force(self):
		...
		self.assertTrue(os.path.isfile(self._ist_file), "Configuration file %s not found" % self._ist_file)
