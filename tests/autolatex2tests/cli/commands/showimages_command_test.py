...
class TestShowImagesAction(AbstractCommandTest):

	def __init__(self, x):
		super().__init__(x, 'showimages')
	...
	def test_valid(self):
		self.do_test('--valid', config=self._config)

	def test_translators(self):
		self.do_test('--translators', config=self._config)

	def test_changed(self):
		self.do_test('--changed', config=self._config)
