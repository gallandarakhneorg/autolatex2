...
class TestCreateBeamerAction(AbstractCommandTest):

	def __init__(self, x):
		super().__init__(x, 'createbeamer')
	...
	def _initialize_test_folder(self):
		...
		self._sty_file = os.path.normpath(os.path.join(self._tmp_folder_name, 'autolatex-beamer.sty'))
	...
	def test_no_existing_file(self):
		...
		self.assertTrue(os.path.isfile(self._sty_file), "STY file %s not found" % self._sty_file)


	def test_existing_file_no_force(self):
		with open(self._sty_file, "wt") as out_file:
			out_file.write("test\n")
