...
class TestUpdateAction(AbstractCommandTest):

	def __init__(self, x):
		super().__init__(x, 'update')
	...
	def test_w_empty_cmd(self):
		self._config.scm.update_cli = []
		with self.assertRaises(Exception):
			self.do_test(config=self._config)

	def test_w_invalid_cmd(self):
		self._config.scm.update_cli = ['invalid-shell-command']
		with self.assertRaises(Exception):
			self.do_test(config=self._config)
	...
	def test_w_valid_cmd(self):
		...
		self._config.scm.update_cli = [command]
		self.do_test(config=self._config)
