...
class TestGloassariesAction(AbstractCommandTest):

	def __init__(self, x):
		super().__init__(x, 'glossaries')
	...
	def _initialize_test_folder(self):
		...
		self._glo_file = os.path.normpath(os.path.join(self._tmp_folder_name, 'rootfile.glo'))
		self._gls_file = os.path.normpath(os.path.join(self._tmp_folder_name, 'rootfile.gls'))
	...
	def test_wo_latex_compilation(self):
		...
		self.assertFalse(os.path.isfile(self._gls_file))

	def test_w_latex_compilation(self):
		maker = AutoLaTeXMaker.create(self._config)
		maker.run_latex(self._tex_file)
		self.assertTrue(os.path.isfile(self._glo_file))
		self.do_test(config=self._config)
		self.assertTrue(os.path.isfile(self._gls_file))
