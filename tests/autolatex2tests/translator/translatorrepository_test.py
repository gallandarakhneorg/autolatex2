...
def generate_translator_stubs_with_conflicts():
	directory = generate_translator_stubs()
...
class TestTranslatorRepositoryShared(AbstractBaseTest):
...
class TestTranslatorRepositoryConflict(AbstractBaseTest):
	...
	def setUp(self):
		logging.getLogger().setLevel(logging.CRITICAL)
		self.directory = generate_translator_stubs_with_conflicts()
		self.config = Config()
		self.config.translators.is_translator_fileformat_1_enable = True
		self.repo = TranslatorRepository(self.config)
		translators = self.repo._read_directory(directory=self.directory, recursive=True, warn=False)
