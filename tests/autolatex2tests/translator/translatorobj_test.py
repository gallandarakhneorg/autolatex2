...
class TestTranslatorS2T(AbstractBaseTest):
	...
	def test_fullSource(self):
		self.assertEqual('s', self.translator.full_source)

	def test_target(self):
		self.assertEqual('t', self.translator.target)

	def test_variante(self):
		self.assertEqual('', self.translator.variant)
...
class TestTranslatorS2TpX(AbstractBaseTest):
	...
	def test_fullSource(self):
		...
		for t2 in targets:
			translator = Translator("s2t+"+t2, self.config)
			with self.subTest(target2=t2):
				self.assertEqual(targets[t2], translator.full_source)
	...
	def test_variante(self):
		...
		for t2 in targets:
			translator = Translator("s2t+"+t2, self.config)
			with self.subTest(target2=t2):
				self.assertEqual('', translator.variant)
...
class TestTranslatorS2TuV(AbstractBaseTest):
	...
	def test_variante(self):
		self.assertEqual('v', self.translator.variant)
...
class TestTranslatorS2TpXuV(AbstractBaseTest):
	...
	def test_variante(self):
		...
		for t2 in targets:
			translator = Translator("s2t+"+t2+"_v", self.config)
			with self.subTest(target2=t2):
				self.assertEqual('v', translator.variant)
...
class TestTranslatorRepositoryInit(AbstractBaseTest):
	...
	def setUp(self):
		logging.getLogger().setLevel(logging.CRITICAL)
		self.directory = generate_translator_stubs()
		self.config = Config()
		self.config.translators.is_translator_fileformat_1_enable = True
		self.repo = TranslatorRepository(self.config)

	def tearDown(self):
		if self.autodelete and self.directory is not None:
