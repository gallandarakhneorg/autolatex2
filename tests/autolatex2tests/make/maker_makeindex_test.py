...
class TestMakeindexMaker(AbstractBaseTest):
	...
	def __install_test_environment(self, valid : bool):
		...
		self.__index_file = os.path.normpath(os.path.join(self.__tmp_folder_name, 'rootfile.idx'))
		...
		if valid:
			shutil.copyfile(os.path.normpath(os.path.join(self.__resource_directory, 'test17.tex')), self.__root_file)
		else:
	...
	def test_run_makeindex_valid(self):
		self.__install_test_environment(True)
		err = self.__maker.run_makeindex(self.__index_file)
		#self.___printlogs()
