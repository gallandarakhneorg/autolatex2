...
class TestMakeglossariesMaker(AbstractBaseTest):
	...
	def __install_test_environment(self, valid : bool):
		...
		if valid:
			shutil.copyfile(os.path.normpath(os.path.join(self.__resource_directory, 'test18.tex')), self.__root_file)
		else:
	...
	def test_run_makeglossaries_invalid(self):
		self.__install_test_environment(False)
		err = self.__maker.run_makeglossaries(self.__root_file)
		#self.___printlogs()
		self.assertFalse(err)

	def test_run_makeglossaries_valid(self):
