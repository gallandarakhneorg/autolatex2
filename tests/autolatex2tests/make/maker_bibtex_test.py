...
class TestBibTeXMaker(AbstractBaseTest):
	...
	def test_run_bibtex_invalid(self):
		self.__install_test_environment(False)
		err = self.__maker.run_bibtex(self.__root_file)
		#self.___printlogs()
		self.assertIsNotNone(err)
		self.assertEqual(self.__aux_file, err['filename'])
		self.assertIn('I found no \\citation commands', err['message'])
		self.assertIn('I found no \\bibdata command', err['message'])
		self.assertIn('I found no \\bibstyle command', err['message'])

	def test_run_bibtex_valid(self):
