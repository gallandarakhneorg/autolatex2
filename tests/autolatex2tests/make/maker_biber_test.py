...
class TestBiberMaker(AbstractBaseTest):
	...
	def __install_test_environment(self, valid : bool):
		...
		self.__base_file = os.path.normpath(os.path.join(self.__tmp_folder_name, 'rootfile'))
		...
		if valid:
			shutil.copyfile(os.path.normpath(os.path.join(self.__resource_directory, 'test16.tex')), self.__root_file)
			shutil.copyfile(os.path.normpath(os.path.join(self.__resource_directory, 'test16.bib')), self.__bib_file)
		else:
	...
	def test_run_biber_invalid(self):
		self.__install_test_environment(False)
		err = self.__maker.run_bibtex(self.__root_file)
		#self.___printlogs()
		self.assertIsNotNone(err)
		self.assertEqual(self.__base_file, err['filename'])
		self.assertIn("Cannot find 'rootfile.bcf'", err['message'])

	def test_run_biber_valid(self):
		self.__install_test_environment(True)
		err = self.__maker.run_bibtex(self.__root_file)
		#self.___printlogs()
