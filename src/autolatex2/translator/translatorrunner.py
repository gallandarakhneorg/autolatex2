...
class TranslatorRunner:
	...
	def get_source_images(self) -> set[str]:
		...
		if self.__images is None:
			...
			for image_directory in self.configuration.translators.image_paths:
				logging.debug(_T("Detecting images inside '%s'") % image_directory)
		...
		if ln == 0:
			...
			if output_filename is not None:
				...
				if len(out_candidates) == 1:
					...
					selected = out_candidates[0]
					logging.warning(_T("Too much translators for file '%s': %s") % (os.path.basename(input_filename), '; '.join(str(element) for element in out_candidates)))
					logging.warning(_T("Selecting translator: %s") % (str(selected)))
			else:
				selected = candidates[0]
				logging.warning(_T("Too much translators for file '%s': %s") % (os.path.basename(input_filename), '; '.join(str(element) for element in candidates)))
				logging.warning(_T("Selecting translator: %s") % (str(selected)))
		return selected
	...
	def get_temporary_files(self, *, in_file: str, translator_name: str = None, out_file: str = None,
		...
		translator = None
		if translator_name:
			translator = self._repository._get_object_for(translator_name)
		if translator is None:
			translator = self.get_translator_for(in_file)
		if translator is None:
			raise TranslatorError(_T("Unable to find a translator for the source image %s") % in_file)
	...
	def get_target_files(self, *, in_file : str, translator_name : str = None, out_file : str = None,
						 pdf_mode : bool = None, fail_on_error : bool = True) -> list[str]:
		"""
	...
	def generate_image(self, *, in_file : str, translator_name : str = None, out_file : str = None, only_more_recent : bool = True, pdf_mode : bool = None, fail_on_error : bool = True, ignore_debug_feature : bool = False) -> str | None:
		"""
		...
		if translator_name:
			translator = self._repository._get_object_for(translator_name)
			if not translator:
				raise TranslatorError(_T("Translator %s was not found but it is mandatory for converting") % translator_name)
		if translator is None:
		...
		if not os.access(in_file, os.R_OK):
			errmsg = _T("%s: file not found or not readable.") % in_file
			if fail_on_error:
		...
		if only_more_recent:
			...
			if out_change is not None and in_change <= out_change:
				...
				logging.log(LogLevel.FINE_INFO, _T("%s is up-to-date") % (os.path.basename(out_file)))
				return None

		logging.info(_T("%s -> %s") % (os.path.basename(in_file), os.path.basename(out_file)))
		...
		logging.debug(_T("Translator: %s") % translator.name)
		logging.debug(_T("In: %s") % str(in_file))
		logging.debug(_T("Out: %s") % str(out_file))
		logging.debug(_T("Pdf: %s") % str(current_pdf_mode))
		...
		if not ignore_debug_feature and debug_translator.python_translator_debugger_enable():
			...
			if logging.getLogger().isEnabledFor(logging.DEBUG):
				...
				logging.debug(_T("Run: %s") % ' '.join(sh_cmd))
			...
			if not exec_env['interpreter_object'].runnable:
				errmsg = _T("Cannot execute the translator '%s'.") % translator_name
				if fail_on_error:
					raise TranslatorError(errmsg)
				else:
					logging.error(errmsg)
			...
			errmsg = _T("Unable to find the method of translation for '%s'.") % translator_name
			if fail_on_error:
				raise TranslatorError(errmsg)
			else:
				logging.error(errmsg)
