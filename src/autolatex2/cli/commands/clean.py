...
class MakerAction(AbstractMakerAction):
	...
	def _is_deletable_in_root_folder_only(self,  root_dir : str,  tex_filename : str,  filename : str) -> bool:
		...
		"""
		basename = os.path.join(root_dir, tex_filename)
		candidates = [
			os.path.normpath(os.path.join(root_dir, '.autolatex_stamp')),
			os.path.normpath(os.path.join(root_dir, 'autolatex_stamp')),
			os.path.normpath(os.path.join(root_dir, 'autolatex_exec_stderr.log')),  # For old version of the program
			os.path.normpath(os.path.join(root_dir, 'autolatex_exec_stdout.log')), # For old version of the program
			os.path.normpath(os.path.join(root_dir, 'autolatex_exec_stdin.log')), # For old version of the program
			os.path.normpath(os.path.join(root_dir, 'autolatex_autogenerated.tex')),
			FileType.pdf.ensure_extension(basename),
			FileType.dvi.ensure_extension(basename),
			FileType.xdvi.ensure_extension(basename),
			FileType.xdv.ensure_extension(basename),
			FileType.ps.ensure_extension(basename),
			genutils.ensure_filename_extension(basename, '.synctex.gz'),
			genutils.ensure_filename_extension(basename, '.synctex'),
		]
	...
	def _is_deletable_shell(self,
		...
		"""
		Replies if the given filename Unix Shell pattern is for a deletable file anywhere.
		"""
