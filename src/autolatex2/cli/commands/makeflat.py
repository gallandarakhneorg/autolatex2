...
class MakerAction(AbstractMakerAction):
	...
	help : str = T('Create a version of the document inside the specified directory (by default \'flat_version\') in which there is a single TeX file, and '
	...
	def _add_command_cli_arguments(self, command_name : str, command_help : str | None,
		...
		self.parse_cli.add_argument('--externalbiblio',
			action = 'store_true',
			help = T('Force the use of an external BibTeX file (i.e. \'.bib\' file) instead of inlining the bibliography database inside the TeX file'))

		self.parse_cli.add_argument('--out',
			default = 'flat_version',
			metavar = 'DIRECTORY',
			help = T('Specify the output directory in which the flat version is created. By default, the name of the directory is \'flat_version\''))
	...
	def run(self, cli_arguments : Namespace) -> bool:
		...
		for root_file in maker.root_files:
			...
			logging.debug(T("Generating flat version into: %s") % output_dir)
			...
			try:
				...
				logging.error(T("Unable to delete the output folder %s: %s") % (output_dir, str(ex)))
				return False
