...
class DynamicBuilder(Builder):
	"""
	Build the bibliography BBL file.
	"""
	...
	def build(self, root_file : str, input_file : FileDescription, maker : TeXMaker) -> bool:
		...
		:rtype: bool
		"""
		if maker.configuration.generation.is_biblio_enable:
			# Ensure Biber configuration
			maker.configuration.generation.is_biber = input_file.use_biber
			error = maker.run_bibtex(input_file.input_filename)
			if error:
	...
	def need_rebuild(self, current_file: FileDescription, dependency_file: FileDescription|None,
		...
		if current_file.use_biber:
			...
			old_md5 = maker.stamp_manager.get_biber_stamp(current_file.output_filename)
			current_md5 = maker.stamp_manager.parse_biber(current_file.output_filename, update_stamps=False)
			if current_md5 != old_md5:
				maker.stamp_manager.set_biber_stamp(current_file.output_filename, current_md5)
				return True
		else:
			# Parse the AUX file to detect the citations
			old_md5 = maker.stamp_manager.get_bibtex_stamp(current_file.output_filename)
			current_md5 = maker.stamp_manager.parse_bibtex(current_file.output_filename, update_stamps=False)
			if current_md5 != old_md5:
				maker.stamp_manager.set_bibtex_stamp(current_file.output_filename, current_md5)
				return True
		return False
